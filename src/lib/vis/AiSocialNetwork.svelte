<script lang="ts">
  ...
  onMount(async () => {
    ...
    const ssIDs = new Set(topicsRaw.filter(isSocialScience).map((d) => d.work_id));
    ...
    topicsFiltered = topicsRaw.filter(
      (d) =>
        intersection.has(d.work_id) &&
        (isAI(d) || isSocialScience(d))
    );
  ...
  function drawNetwork() {
    ...
    const simulation = d3
      ...
      .force("collision", d3.forceCollide().radius((d) => sizeScale(d.count) + 8))
